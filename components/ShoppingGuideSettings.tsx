...
interface SettingsProps {
    ...
    onUpdateInfo: (info: ShoppingGuideInfo) => void;
}
...
const ShoppingGuideSettings: React.FC<SettingsProps> = ({ currentInfo, onUpdateInfo }) => {
    const [info, setInfo] = useState(currentInfo);
    const [isSaving, setIsSaving] = useState(false);
    const [imageFile, setImageFile] = useState<File | null>(null);
    const [previewUrl, setPreviewUrl] = useState<string | null>(currentInfo.imageUrl);

    useEffect(() => {
        setInfo(currentInfo);
        setPreviewUrl(currentInfo.imageUrl)
    }, [currentInfo]);
    ...
    const isUnchanged = JSON.stringify(info) === JSON.stringify(currentInfo) && !imageFile;

    return (
        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-md sticky top-8">
            <h3 className="text-xl font-semibold text-gray-900">頁面內容</h3>
            <p className="mt-1 text-sm text-gray-600">編輯顯示在此頁面上的內容。</p>
                ...
                <div>
                 ...
                 <div>
                    ...
                    <div className="flex items-center justify-center w-full mt-1">
                        <label htmlFor="guide-image-upload" className="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                            {previewUrl ? (
                            ...
                            <input id="guide-image-upload" type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
                        </label>
                    </div>
                </div>
                <button
...
export default ShoppingGuideSettings;