...
const ContactPageSettings: React.FC<ContactPageSettingsProps> = ({ currentContactPageInfo, onUpdateContactPageInfo }) => {
    const [info, setInfo] = useState(currentContactPageInfo);
    const [isSaving, setIsSaving] = useState(false);
    const [imageFile, setImageFile] = useState<File | null>(null);
    const [previewUrl, setPreviewUrl] = useState<string | null>(currentContactPageInfo.imageUrl);

    useEffect(() => {
        setInfo(currentContactPageInfo);
        setPreviewUrl(currentContactPageInfo.imageUrl)
    }, [currentContactPageInfo]);
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            onUpdateContactPageInfo(finalInfo);
    ...
    const isUnchanged = JSON.stringify(info) === JSON.stringify(currentContactPageInfo) && !imageFile;

    return (
        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-md sticky top-8">
            <h3 className="text-xl font-semibold text-gray-900">「聯絡我們」頁面內容</h3>
            <p className="mt-1 text-sm text-gray-600">編輯顯示在「聯絡我們」頁面上的內容。</p>
                ...
                <div>
                 ...
                 <div>
                    ...
                    <div className="flex items-center justify-center w-full mt-1">
                        <label htmlFor="contact-image-upload" className="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                            {previewUrl ? (
                                <img src={previewUrl} alt="Contact Us Preview" className="object-cover w-full h-full" />
                            ) : (
                            ...
                            <input id="contact-image-upload" type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
                        </label>
                    </div>
                </div>
                <button
